...
public class CustomerController {
    ...
    @ApiOperation(value = "View a list of available active customers", response = CustomerList.class)
    @CrossOrigin
    @GetMapping(value = "/active", produces = "application/json")
    public ResponseEntity<CustomerList> getAllActiveCustomers(@NotNull @Min(0) @ApiParam(value = "The number of items to skip before starting to collect the result set.", required = true, defaultValue = "0") @RequestParam(value = "offset", required = true, defaultValue = "0") Integer offset, @NotNull @Min(1) @Max(100) @ApiParam(value = "The numbers of items to return.", required = true, defaultValue = "10") @RequestParam(value = "limit", required = true, defaultValue = "10") Integer limit) {
        return ResponseCreator.successfulResponse(customerService.getAllActiveCustomers(new PageDetails(new SearchFilter(), offset, limit)));
    }
